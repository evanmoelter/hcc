...
2. Create Apollo's independent Cloudflare tunnel and `external-apollo.${SECRET_DOMAIN}` alias.
   ...
   Before enabling the tunnel, restrict public access to echo-server for testing; remove its external
   Gateway attachment when those tests finish so it does not remain an unauthenticated public endpoint.
...
4. Test the split-horizon and dual-route options in
   ...
   Set `external-dns.alpha.kubernetes.io/target` on the Gateway, not on individual routes.
   That annotation supplies one target to every watcher; it does not by itself
