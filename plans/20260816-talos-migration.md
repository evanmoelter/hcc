...
| Mealie | **Missing its internal exposure.** `food.${SECRET_DOMAIN}` has only an `external` ingress, while echo-server and Authentik both carry internal routes alongside their external ones — an omission from when internal DNS went out of service, not a deliberate choice | Restore LAN reachability during the rebuild. Under split-horizon this is automatic (the UniFi instance answers from the external Gateway); under dual-route it needs an internal route authored explicitly. Also worth adding a `tailscale` Ingress, which it lacks too |
...
**The standing policy is that every externally-published app is also reachable directly on the LAN.** Only machine endpoints are genuinely external-only: the Flux GitHub webhook receiver and Authentik's apex webfinger path, neither of which a browser on the LAN needs. So "dual-exposed" is the normal case for user-facing apps, not a special case for the IdP — which is what makes the choice below worth making deliberately rather than per-app.
...
- **Every app gets a direct LAN path for free.** Given the policy that externally-published apps are also LAN-reachable, dual-route needs two routes for *every* such app, not just the IdP — so this is an object-count saving across the whole external set, not a one-off.
